...
(function ($) {
    ...
    $.fn.slotMachine = function (settings) {
         ...
         * @return int - Element index and HTML node
         */
        function _getNext(index) {
            index = index === undefined ? _active.index : index;
            var nextIndex = index + 1 < $titles.length ? index + 1 : 0;
            var nextObj = {
                index: nextIndex,
         ...
         */
        function _shuffle(count, stopIndex) {
        ...
        function _stop(nowOrRepeations, getElementFn) {
            ...
            } else if (typeof getElementFn === "number") {
                rnd = _getNext(getElementFn);
            } else {
                if (settings.repeat) {
                    rnd = _getNext();
                } else {
            ...
            if (nowOrRepeations === true || nowOrRepeations <= 1) {
                ...
                _shuffle(nowOrRepeations || 3, getElementFn);
         ...
         * @param int||boolean nowOrRepeations - Number of repeations to stop (true to stop NOW)
         */
        $slot.stop = function (nowOrRepeations, stopIndex) {
            ...
            _stop(nowOrRepeations, stopIndex);
         ...
         */
        $slot.next = function (index) {
            if (typeof(index) == "undefined" || index >= $titles.length) {
                _stop(true, _getNext);
            } else {
                _stop(true, index);
            }
